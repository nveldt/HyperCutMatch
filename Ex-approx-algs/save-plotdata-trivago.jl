...
mat_sdp = matread("Output/trivago/trivago_$(SetInds[num])_SDP_solution.mat")
...
for num = 1:length(SetInds)
    ...
    mat_cm = matread("Output/trivago/trivago_$(SetInds[num])_HCM_solution.mat")
    ...
    mat_lp = []
    try
        mat_lp = matread("Output/trivago/trivago_$(SetInds[num])_LP_solution_notimelimit.mat")
    catch
        mat_lp = matread("Output/trivago/trivago_$(SetInds[num])_LP_solution.mat")
    end
    # mat_lp = matread("Output/trivago/trivago_$(SetInds[num])_LP_solution.mat")
    lb = mat_lp["lb"]
    ...
    if optimal
        ...
        push!(exp_lp,exp1)
        println("$num LP optimal")
    else
        println("$num LP not optimal")
        push!(approx_lp,-1)
        push!(runs_lp,-1)
        push!(exp_lp,-1)
    end
    ...
    try
        ...
        # println("$(SetInds[num]) not for SDP")
        push!(approx_sdp,-1)
        push!(runs_sdp,-1)
        push!(exp_sdp,-1)
        push!(bestapp_sdp, -1)
...
##
matwrite("plotdata_trivago_notimelimitlp.mat", Dict("Hypstats"=>Hypstats,
