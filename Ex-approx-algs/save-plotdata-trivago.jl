...
# Manual report of how the SDP solver did
...
for num = 1:length(SetInds)
    ...
    mat_cm = matread("Output-CM-trivago/trivago_$(SetInds[num])_output.mat")
    ...
    try 
        mat_cm = matread("Output-LR-Trivago/trivago_$(SetInds[num])_output.mat")
        X = mat_cm["X"]
        X = X+X'
        lb = mat_cm["lb"]
    ...
    try
        mat_sdp = matread("../data/main-data/trivago/trivago_$(SetInds[num])_SDP_solution.mat")
        X = mat_sdp["X"]
        X = X+ X'
...
matwrite("plotdata_trivago.mat", Dict("Hypstats"=>Hypstats,"approx_cm"=>approx_cm, "runs_cm"=>runs_cm, "approx_lp"=>approx_lp, "runs_lp"=>runs_lp, "approx_sdp"=>approx_sdp, "runs_sdp"=>runs_sdp))