...
f = readlines("../data/arb-data/$dataset/node-labels-$dataset.txt")
n = length(f)
...
f = readlines("../data/arb-data/$dataset/hyperedges-$dataset.txt")
m = length(f)
...
f = readlines("../data/arb-data/$dataset/label-names-$dataset.txt")
t = length(f)
...
matwrite("mathoverflow-answers-all.mat", Dict("H"=>H,"LabelNames"=>LabelNames,"Nodes2Labels"=>Nodes2Labels))
...
## Save a range of small hypergraphs at various sizes
...
for i = 1:length(SetInds)
    ...
    topic = Names[l]
    push!(topics,topic)
    matwrite("../data/main-data/mathoverflow/math_$(l)_lcc.mat", Dict("H"=>Hq,"topic"=>topic))

end

## Save the data
matwrite("../data/main-data/mathoverflow/math_summary.mat",Dict("SetInds"=>SetInds,"stats"=>stats,"topics"=>topics,"statkey" => "num nodes, num edges, sum of edge sizes, max hyperedge size, mean hyperedge size"))
